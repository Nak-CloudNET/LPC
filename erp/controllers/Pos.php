...
class Pos extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
		...
		$add_receive_link = anchor('pos/add_receive/$1', '<i class="fa fa-money"></i> ' . lang('add_receive'), 'data-toggle="modal" data-target="#myModal"');
		...
		$tmp_table = "(
				SELECT
					...
					(
						CASE
						WHEN total_receive = 0 THEN
							'pending'
						ELSE
							CASE
						WHEN total_receive < total_qty THEN
							'rec_partial'
						ELSE
							'received'
						END
				...
				FROM
					(
						SELECT
							...
							count(erp_sale_items.id) AS total_qty,
							sum(erp_sale_items.received) AS total_receive,
							erp_sale_items.sale_id,
          ...
          where jobs ='1'
				...
				LEFT JOIN (
					SELECT
						...
						erp_sale_dev_items.sale_id
					FROM
						erp_sale_dev_items
					GROUP BY
						erp_sale_dev_items.sale_id
				) AS erp_sale_dev_items ON erp_sale_dev_items.sale_id = erp_sale_items.sale_id
			) AS erp_sale_items";
        if ($warehouse_id) {
			...
			$this->datatables
				...
				MAX(".$this->db->dbprefix('payments').".date) as pdate,
				".$this->db->dbprefix('sales').".reference_no, biller, customer, users.username AS saleman, grand_total, paid, (grand_total - paid) AS balance, payment_status, dev_status, receive_status")
                ->from('sales')
				->join('payments', 'payments.sale_id=sales.id', 'left')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
				->join('users', 'users.id=sales.saleman_by', 'left')
				->join($tmp_table, 'erp_sale_items.sale_id=sales.id', 'left')
    ...
    /* ---------------------------------------------------------------------------------------------------- */
	function add_receive($id = NULL){
		$status =$this->pos_model->checkDevStatus($id);
		//$this->erp->print_arrays($status);
		if($this->input->post('add_receive') !='')
		{
			...
			for($i=0; $i <= count($checked);$i++ ){
				...
				$update = $this->db->where(array('id'=>$checked[$i]))->update('erp_sale_items',$data);
			}
			if($update){
				redirect('pos/sales');
			}
		...
		else
		{
			...
			$this->data['id']=$id;
			$this->data['dev_status'] = $status;
			$this->data['rows'] = $this->pos_model->getAllInvoiceItems($id);
			$this->load->view($this->theme . 'pos/add_receive', $this->data);
		}
	}
    function index($sid = null, $sale_order_id=null, $combine_table = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $curr_payment = $this->input->post('curr_payment');
			$payment_status = 'due';
            $payment_term = 0;
    ...
    function view_old($sale_id = NULL, $modal = NULL)
    {
        $this->erp->checkPermissions('index');
        if ($this->input->get('id')) {
            $sale_id = $this->input->get('id');
	...
	function view($id = NULL, $isModal = NULL)
    {
		$this->erp->checkPermissions('index');
        ...
        $this->data['rows'] = $this->sales_model->getAllInvoiceItemsDev($id);
		$this->data['payment'] = $this->sales_model->getPaymentBySaleID($id);
		$this->data['logo'] = true;
		$this->data['isModal'] = $isModal;
        $this->load->view($this->theme . 'pos/invoice_photo', $this->data);
    }
