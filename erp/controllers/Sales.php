...
class Sales extends MY_Controller
{
	...
	function modal_view($id = NULL)
    {
        ...
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : NULL;
        // $this->erp->print_arrays($this->data['updated_by']);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$date_receive= $this->erp->fld(trim($this->input->post('date_receive')));
   ...
   public function combine_pdf($sales_id)
        ...
        if ($this->form_validation->run() == true) {
            if($str == "1"){
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $sum_qty = 0;
                    foreach ($_POST['val'] as $id) {
                        $delivery = $this->sales_model->getDeliveryByID($id,$wh);
                        $sum_qty += $delivery->qty;
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->erp->hrld($delivery->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $delivery->do_reference_no." ");
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $delivery->sale_reference_no." ");
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $sum_qty = 0;
                    foreach ($_POST['val'] as $id) {
                        $delivery = $this->sales_model->getOrderDeliveryByID($id,$wh);
                        $sum_qty += $delivery->qty;
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->erp->hrld($delivery->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $delivery->do_reference_no." ");
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $delivery->sale_reference_no." ");
    ...
    function suggestions()
    {
		...
		$kh_currency = $this->sales_model->getKHMCurrency();
		if ($rows) {
            foreach ($rows as $row) {
				...
				//if($row->currentcy_code == "KHM"){
				//	$row->price = $row->price / $kh_currency->rate;
				//}
    ...
    function suggestionsSale()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				//$this->erp->print_arrays($percent->percent);
