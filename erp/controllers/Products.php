...
class Products extends MY_Controller
{
	...
	*/	
		...
		$price_group ="(SELECT
				...
				FROM
					erp_product_prices
				INNER JOIN erp_price_groups ON erp_price_groups.id = erp_product_prices.price_group_id) as group_price";
        if($this->Owner || $this->Admin ){
            if ($warehouse_id) {
                $this->datatables
				...
				->join($price_group, 'group_price.product_id = erp_products.id', 'left')
            ...
            if ($warehouse_id) {
                ...
                $this->db->dbprefix('products') . ".name as name, " . 
                $this->db->dbprefix('products') . ".name_kh as kname, " .
                $this->db->dbprefix('categories') . ".name as cname,subcategories.name as sub_name, COALESCE(sum(wp.quantity), 0) as quantity, ".
                $this->db->dbprefix("units").".name as unit, alert_quantity", FALSE)        
                ->from('products');
    ...
    function add($id = NULL,$param=null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$commission = array(				
				'photowriter'		=> $this->input->post('photowriter'),
				'computer'			=> $this->input->post('computer'),
				'developphoto'		=> $this->input->post('developphoto'),
				'photographer'		=> $this->input->post('photographer'),
				'studio_decor'		=> $this->input->post('studio_decor'),
        ...
        if ($this->form_validation->run() == true && $pid = $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $related_products, $commission)) {
			...
			$this->data['currencies'] = $this->products_model->getAllCurrency();
            $this->data['categories'] = $this->site->getAllCategories();
    ...
    function edit($id = NULL)
    {
        ...
        $product = $this->site->getProductCommission($id);
        if (!$id || !$product) {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $related_products, $commission)) {
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $ikeys = array('code', 'name', 'name_kh', 'category_id', 'unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_id', 'variants', 'supplier1','supplier2','supplier3', 'supplier4', 'supplier5', 'cf1', 'cf2', 'currentcy_code', 'cf4', 'cf5', 'cf6', 'image', 'type');
...
//                              $this->erp->print_arrays($data);
                                ...
                                $this->excel->getActiveSheet()->SetCellValue('F' . $a, $product->quantity);
                                $this->excel->getActiveSheet()->SetCellValue('G' . $a, $product->unit);
                               $this->excel->getActiveSheet()->getStyle('G'. $a)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                                $this->excel->getActiveSheet()->SetCellValue('H' . $a, $product->alert_quantity);
                                    
                                //to display total sum
                                $i = $a+1;
                                $this->excel->getActiveSheet()->SetCellValue('F' . $i, $sum_quantity);
                            }
							$a++;
                        }
						//apply style border top and bold text in case excel
	...
	public function product_analysis($id = null)
    {
        ...
        $this->data['group1'] = $this->site->getUser($header->group1);
        $this->data['group2'] = $this->site->getUser($header->group2);
        $this->data['group3'] = $this->site->getUser($header->group3);
        $this->data['group4'] = $this->site->getUser($header->group4);
        $this->data['group5'] = $this->site->getUser($header->group5);
    ...
    { 
		...
		$this->data['created_by'] 		= $this->products_model->getUserByGroup();
