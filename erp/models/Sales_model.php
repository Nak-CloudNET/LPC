...
class Sales_model extends CI_Model
{
	...
	public function getAllInvoiceItemsDev($sale_id)
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.unit, products.currentcy_code, products.promotion, categories.name AS category_name,sale_dev_items.quantity as dev_qty')
            ->join('products', 'products.id=sale_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
			->join('categories', 'categories.id = products.category_id', 'left')
			->join('erp_sale_dev_items', 'erp_sale_dev_items.sale_id = erp_sale_items.sale_id AND erp_sale_dev_items.product_id=erp_sale_items.product_id', 'left')
  ...
  //           ->group_by('deliveries.id')
	...
	public function getKHMCurrency(){
		$q  = $this->db->get_where("erp_currencies", array("code" => "KHM")); 
		if($q->num_rows()>0){
			return $q->row();
		}
		return false;
