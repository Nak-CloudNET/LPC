...
class Pos_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id, products.code, products.name, products.type,categories.type AS cate_type,warehouses_products.product_id, warehouses_products.quantity, price, tax_rate, tax_method,products.image, currentcy_code ,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`)
    ...
    */
						...
						foreach($items as $item){
							...
							if($data->quantity == $item['qty']){
								...
								if($this->db->insert("suspended_items",$data)){
									...
									$this->db->update("suspended_items",array('quantity'=>$qq,'subtotal'=>$s->subtotal,'suspend_id'=>$dd->id),array("id"=>$item['id']));
								}
							}
						}
	...
	public function updateprinted($susp_id,$item_id)
    {
		...
		public function checkDevStatus($id=NULL){
		$bq ="SELECT
					(
						CASE
						WHEN IFNULL(
          ...
          where jobs ='1'
				...
				WHERE erp_sale_items.sale_id='".$id."'";
		$q = $this->db->query($bq, false);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
	}
