...
class Site extends CI_Model
{
    ...
    public function get_total_qty_alerts() {
        $this->db->where('quantity <= alert_quantity', NULL, FALSE)->where('track_quantity', 1)->where('type !=', 'service');
 ...
 saram	public function getSuspendByID($id){
	...
	*/
    ...
    public function getProductByID($id) {
        $this->db->select('products.*,products.quantity AS qoh, units.name as unit, products.unit as unit_id,categories.type as cate_type, commission.photowriter, commission.computer, commission.photographer, commission.studio_decor, commission.developphoto, commission.group1, commission.group2, commission.group3, commission.group4, commission.group5');
		$this->db->join('categories','categories.id=products.category_id','left');
        $this->db->join('units', 'units.id = products.unit', 'left');
        $this->db->join('commission', 'commission.product_id = products.id', 'left');
	...
	public function getProductCommission($id) {
        $this->db->select('products.*, commission.salesman,commission.photowriter, commission.computer, commission.photographer, commission.studio_decor, commission.developphoto, commission.group1, commission.group2, commission.group3, commission.group4, commission.group5');  
		$this->db->join('commission', 'commission.product_id = products.id', 'left');
        $q = $this->db->get_where('products', array('products.id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
	...
	public function getAllUsersByGroup($id) {
	...
	public function getAllWarehouseProducts() {
		$this->db->select('warehouses_products.*, warehouses.name');
	...
	public function syncSalePayments($id) {
		...
		if($this->get_setting()->stock_deduction=='delivery'){
			if ($this->db->update('sales', array('paisd' => $paid,'payment_status' => $payment_status,'payment_term'=>$payment_term), array('id' => $id))) {
				return true;
			}
		}else{
	...
	*/
	...
	function alertUnfinishedPhoto(){
		$q ="SELECT
					count(*) as alert_unifinish_goods
				FROM
					erp_sales
				INNER JOIN (
					SELECT
					...
					FROM
						erp_sale_items
					LEFT JOIN erp_sale_dev_items ON erp_sale_dev_items.sale_id = erp_sale_items.sale_id
					AND erp_sale_items.product_id = erp_sale_dev_items.product_id
					INNER JOIN erp_products ON erp_products.id = erp_sale_items.product_id
					INNER JOIN erp_categories ON erp_categories.id = erp_products.category_id
					WHERE
						jobs = '1'
				) AS b ON b.sale_id = erp_sales.id
				WHERE
		...
		$resu = $this->db->query($q)->row();
		return $resu->alert_unifinish_goods;
