...
class Products_model extends CI_Model
{
	...
	public function getConvertByID($id) {
        $this->db->select("convert.id, 
					...
					CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user, convert.created_by, warehouses.id as warehouse_id, warehouses.name as war_name, convert.biller_id, convert.bom_id, convert.group1, convert.group2, convert.group3, convert.group4, convert.group5")
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $related_products=NULL, $commission)
    {
		...
		if ($this->db->insert('products', $data)) {
			...
			if($commission){
				...
				$this->db->insert('commission', $commission);
			}
    ...
    public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $related_products=NULL, $commission)
    {
		if ($this->db->update('products', $data, array('id' => $id))) {
			if($commission){
				$this->db->update('commission', $commission, array('product_id' => $id));
			}
            if ($items) {
    ...
    {
        if ($this->db->delete('products', array('id' => $id)) && $this->db->delete('warehouses_products', array('product_id' => $id)) && $this->db->delete('warehouses_products_variants', array('product_id' => $id)) && $this->db->delete('commission', array('product_id' => $id))) {
            return true;
        }
        return FALSE;
    }
    ...
    */
	...
	{
		$this->db->select('bom.*, bom_items.*, (SELECT SUM(COALESCE(quantity , 0)) as qoh FROM erp_warehouses_products WHERE warehouse_id = '.$ware_id.' AND erp_warehouses_products.product_id = erp_bom_items.product_id) as qoh')
    ...
    {
        $q = $this->db->get_where('currencies', array('code <>' => 'KHM_o'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
