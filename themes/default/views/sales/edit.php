...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        localStorage.setItem('date_receive', '<?= $this->erp->hrld($inv->date_receive) ?>');
		...
		$(document).on('change', '#date_receive', function (e) {
            localStorage.setItem('date_receive', $(this).val());
        });
        if (date_receive = localStorage.getItem('date_receive')) {
            $('#date_receive').val(date_receive);
        }
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin) { ?>
							...
							<div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("date_receive", "date_receive"); ?>
                                    <?php echo form_input('date_receive', (isset($_POST['date_receive']) ? $_POST['date_receive'] : $this->erp->hrld($inv->date_receive)), 'class="form-control input-tip datetime" id="date_receive" required="required"'); ?>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="col-md-4">
