...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		$.each(positems, function (i, e) {
			...
			sep 				= item.row.sep; 
			var p_kh_rate 		= localStorage.getItem('exchange_kh');
			if(item.row.currentcy_code == "KHM"){
				var unit_price 		= item.row.real_unit_price / p_kh_rate;
				var real_unit_price = item.row.real_unit_price / p_kh_rate;	
			}else{
				var unit_price 		= item.row.real_unit_price;
				var real_unit_price = item.row.real_unit_price;
			}
			console.log(item.row.currentcy_code+'/////'+unit_price+'/////'+real_unit_price);
			var default_price 	= 0;
			...
			var p_kh_price = '៛ ' + (parseFloat(real_unit_price * p_kh_rate).toFixed(0));
			if (site.settings.product_serial == 1) {
			...
			if (item_promotion == 1){
				...
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + real_unit_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + real_unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + real_unit_price + '"><input class="currency_code" name="currency_code[]" type="hidden" value="' + item.row.currentcy_code + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(real_unit_price)) + '</span></td>';
			}
			
			tr_html += '<input class="default_price" name="default_price[]" type="hidden" value="' + default_price + '">';
			
			tr_html += '<td class="text-right"><span class="price-kh">'+ formatMoney(p_kh_price) +'</span></td>';
